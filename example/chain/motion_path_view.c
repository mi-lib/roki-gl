...
/* the main data of this sample code */
typedef struct{
  char *modelfile;
  /* the main inputs for pathplan */
  keyFrameInfoArray keyframe_array; /* size = keyframe size */
  p2pPathArray p2p_array; /* size = path size */
...
int createPinInfoDisplayList(rkChain* chain, allLinkPinIKInfo *all_link_pin_ik_info, double alpha, rkglLight* light)
{
  ...
  if( !( oi_alt = zAlloc( zOpticalInfo*, rkChainLinkNum(chain) ) ) ){
    ZALLOCERROR();
    ZRUNERROR( "Failed to zAlloc( zOpticalInfo, rkChainLinkNum(&g_main->chain) )." );
    return -1;
  }
  ...
  for( link_id=0; link_id < rkChainLinkNum( chain ); link_id++ ){
    ...
    rkglChainAlternateLinkOptic( &g_main->glChain, link_id, oi_alt[link_id], light );
...
{
  int i;
  if( zArraySize( &g_main->keyframe_array ) > 0 ){
    for( i=0; i < zArraySize( &g_main->keyframe_array ); i++ ){
      keyFrameInfo* kf = zArrayElemNC( &g_main->keyframe_array, i );
      zVecFree( kf->q );
      zArrayFree( &kf->all_link_pin_ik_info );
    }
    zArrayFree( &g_main->keyframe_array );
  }
}
...
bool clone_and_set_keyframelist(const int input_keyframe_size, double** input_q, int** input_pin, double**** input_ap)
{
  ...
  free_keyframe_array();
  zArrayAlloc( &g_main->keyframe_array, keyFrameInfo, input_keyframe_size );

  rkglChainAttrInit( &attr );
  for( key_id=0; key_id < input_keyframe_size; key_id++ ){
    keyFrameInfo* kf = zArrayElemNC( &g_main->keyframe_array, key_id );
...
void calc_keyframe_feedrate(rkChain *chain, keyFrameInfoArray* keyframe_array)
{
  ...
  zArrayElemNC(keyframe_array, 0)->feedrate.ds = 0.0;
  zArrayElemNC(keyframe_array, 0)->feedrate.d2s = 0.0;
...
void free_p2p_array(void)
{
  ...
  if( (path_size = zArraySize( &g_main->p2p_array )) > 0 ){
    for( path_id=0; path_id < path_size; path_id++ ){
      p2pPathInfo* p2p_buf = zArrayElemNC( &g_main->p2p_array, path_id );
...
bool clone_and_set_ref_path_from_refPathinput(const int path_size, refPathInput* src_ref_path_input_array)
{
  ...
  if( !init_p2p_array( &g_main->chain, &g_main->p2p_array, path_size, IK_PIN_CELL_SIZE ) )
    return false;
  for( path_id=0; path_id < path_size; path_id++ ){
    p2pPathInfo* p2p_buf = &g_main->p2p_array.buf[path_id]; /* for omission */
...
bool interpolate_path()
{
  ...
  p2pPathArray* p2p_array = &g_main->p2p_array;
  ...
  calc_keyframe_feedrate( &g_main->chain, &g_main->keyframe_array );

  set_constrained_mode_for_each_p2p_path( &g_main->keyframe_array, &g_main->p2p_array );
...
{
  rkIKCellSetRefVec( g_main->p2p_array.buf[path_id].ref_path_array.buf[ik_id].cell, ref_pos );
}
...
void set_one_target_reference_of_3D_attitude_position_for_IK(const int path_id, const int ik_id, zMat3D* ref_att)
{
  ...
  rkIKCellSetRefVec( g_main->p2p_array.buf[path_id].ref_path_array.buf[ik_id].cell, &zyx );
}
...
bool pop_pose(const double s, rkChain* chain, p2pPathArray *p2p_array)
{
  ...
  if( range_s < DBL_MIN ){
    ZRUNERROR("range_s = %f is too shord!!", range_s);
    return false;
  }
...
void print_header_label_of_ik_nurbs_path(FILE *fp, const int cell_size)
{
  int cell_id;
...
void print_header_label_of_ik_result_q_path(FILE *fp, const int joint_size)
{
  int jid;
...
void print_interpolated_qref_path(FILE *fp, const double s, zPexIPArray* qref_path, zVec* qref)
{
  int jid;

  *qref = pop_qref( s, qref_path );
  for( jid=0; jid < zVecSize( *qref ); jid++ )
    fprintf( fp, "%.10f ", (*qref)->buf[jid] );
}
...
void print_interpolated_nurbs_path_3d_position(FILE *fp, const double s, zNURBS3D *nurbs, bool is_path, zVec3D* out_pos)
{
  if( is_path ){
    zNURBS3D1Vec( nurbs, s, out_pos );
    fprintf( fp, "%.10f %.10f %.10f ", out_pos->c.x, out_pos->c.y, out_pos->c.z );
  }
  else
    fprintf( fp, "0.0 0.0 0.0 " ); /* fprintf( fp, "NaN NaN NaN " ); */
}

void print_ik_result_q_path(FILE *fp, rkChain* chain, zVec tmp_q){
  int i;
  rkChainGetJointDisAll( chain, tmp_q );
  for( i=0; i<zVecSizeNC(tmp_q); i++ )
    fprintf( fp, "%.10f ", zVecElemNC(tmp_q,i) );
}

void print_interpolated_path(FILE *fp, rkChain* chain, p2pPathArray *p2p_array, const int s_slice_num)
{
  bool is_find;
  int path_id, link_id, cell_id, ik_id;
  int sid;
  double s, pexIP_s, range_s, end_s;

  const int joint_size = zArraySize( &p2p_array->buf[0].qref_path );
  const int all_link_size = p2p_array->buf[0].all_link_size;
  const int cell_size = p2p_array->buf[0].all_link_pin_path_ik_info.buf[0].cell_size;
  const int path_size = zArraySize( p2p_array );
  ...
  print_header_label_of_ik_result_q_path( fp, joint_size );
  print_header_label_of_ik_nurbs_path( fp, cell_size );
  fprintf( fp, "\n" );

  zVec q_out = zVecAlloc( rkChainJointSize( chain ) );
  for( sid=0; sid < s_slice_num; sid++ ){
    ...
    pexIP_s = s - p2p_buf->start_feedrate.s;
    zVec qref;
    print_interpolated_qref_path( fp, pexIP_s, &p2p_buf->qref_path, &qref );

    for( link_id=0; link_id < all_link_size; link_id++ ){
      for( cell_id=0; cell_id < cell_size; cell_id++){
        pinPathIKInfo* cell_info = &p2p_buf->all_link_pin_path_ik_info.buf[link_id].c[cell_id];
        print_interpolated_ik_weight_path( fp, pexIP_s, &cell_info->weight_path );
      }
    }
    ...
    range_s = p2p_buf->goal_feedrate.s - p2p_buf->start_feedrate.s;
    zVec3D ref_pos;
    for( ik_id=0; ik_id < p2p_buf->ik_num; ik_id++ ){
    ...
    update_alljoint_by_IK( chain, qref, is_free_joints_with_no_IK );
    zVecFree( qref );
    print_ik_result_q_path( fp, chain, q_out );
    /* print nurbs path if exsits */
    for( ik_id=0; ik_id < p2p_buf->ik_num; ik_id++ ){
      refPath* ref_path = &p2p_buf->ref_path_array.buf[ik_id]; /* for omission */
      if( g_ik_reg_cls->pos( ref_path->ik_reg ) ){
        const bool is_path = true;
        print_interpolated_nurbs_path_3d_position( fp, (pexIP_s / range_s), &ref_path->nurbs, is_path, &ref_pos );
      }
    }
    fprintf( fp, "\n" );
  ...
  } /* end of for( sid=0; sid < s_slice_num; sid++ ) */
}

void dump_interpolated_path(const char* filename, const int _sliceNUM ){
  FILE *fp;
  int slice_num;
  ...
  print_interpolated_path( fp, &g_main->chain, &g_main->p2p_array, slice_num );
...
double run_test(void)
{
  ...
  dump_interpolated_path( dump_filename, SLICE_NUM );
...
{
  g_main->feedrate_s = s;
  return pop_pose( g_main->feedrate_s, &g_main->chain, &g_main->p2p_array );
}

void release_path(void)
{
  if( g_main->glChain.chain != NULL ){
    rkglChainUnload( &g_main->glChain );
    rkChainDestroy( &g_main->chain );
  }
  free_keyframe_array();
  free_p2p_array();
}
...
void draw_nurbs_in_one_path(void)
{
  int ik_num, ik_id;
  zRGB rgb;

  ik_num = g_main->p2p_array.buf[g_main->draw_path_id].ik_num;
  glPushMatrix();
  for( ik_id=0; ik_id < ik_num; ik_id++ ){
    refPath* ref_path = &g_main->p2p_array.buf[g_main->draw_path_id].ref_path_array.buf[ik_id];
    if( g_ik_reg_cls->pos( ref_path->ik_reg ) ){
      zRGBSet( &rgb, 1.0, 1.0, 1.0 );
      glLoadName( NAME_NURBS + 100*g_main->draw_path_id + ik_id );
      glLineWidth( 3 );
...
void draw_nurbs(void)
{
  ...
  for( g_main->draw_path_id=0; g_main->draw_path_id < path_size; g_main->draw_path_id++ ){
    draw_nurbs_in_one_path();
  }
}
...
void draw_keyframes_phantom_chain(void)
{
  ...
  glLoadName( g_main->glChain.name );
  for( key_id=0; key_id < zArraySize( &g_main->keyframe_array ); key_id++ ){
    glPushName( key_id );
    glCallList( g_main->keyframe_array.buf[key_id].display_id );
    glPopName();
  }
}
...
int find_cp(rkglSelectionBuffer *sb)
{
  ...
  int i, ik_num, selected_ik_id_tmp, selected_cp_tmp;

  /* path_size = zArraySize( &g_main->p2p_array ); */

  rkglSelectionRewind( sb );
  g_main->selected_cp = -1;
  g_main->selected_path_id = -1;
  g_main->selected_ik_id = -1;

  p2pPathInfo* p2p_buf = &g_main->p2p_array.buf[g_main->draw_path_id];
  ik_num = g_main->p2p_array.buf[g_main->draw_path_id].ik_num;

  for( i=0; i<sb->hits; i++ ){
    selected_ik_id_tmp = rkglSelectionName(sb,0) - ( NAME_NURBS + 100*g_main->draw_path_id );
    if( selected_ik_id_tmp >= 0 &&
        selected_ik_id_tmp < ik_num ){
      void* ik_reg = p2p_buf->ref_path_array.buf[selected_ik_id_tmp].ik_reg;
      if( g_ik_reg_cls->pos( ik_reg ) ){
        zNURBS3D* nurbs = &p2p_buf->ref_path_array.buf[selected_ik_id_tmp].nurbs;
        selected_cp_tmp = rkglSelectionName(sb,1);
        if( selected_cp_tmp >= 0 &&
            selected_cp_tmp < zNURBS3D1CPNum(nurbs) ){
          g_main->selected_path_id = g_main->draw_path_id;
          g_main->selected_ik_id = selected_ik_id_tmp;
          g_main->selected_cp = selected_cp_tmp;
          break;
        }
      }
      rkglSelectionNext( sb );
    }
    eprintf(" sp->hits=%d : i=%d : g_main->draw_path_id=%d, ik_id=%d, rkglSeleionName(sb,0)=%d, Name(sb,1)=%d\n", sb->hits, i, g_main->draw_path_id, selected_ik_id_tmp, rkglSelectionName(sb,0), rkglSelectionName(sb,1));
  }
...
int find_keyframes_phantom_chain(rkglSelectionBuffer *sb)
{
  g_main->selected_key_id =
    ( rkglSelectionName(sb,0) == g_main->glChain.name &&
      rkglSelectionName(sb,1) >= 0 &&
      rkglSelectionName(sb,1) < zArraySize( &g_main->keyframe_array ) ) ?
    rkglSelectionName(sb,1) : -1;
  return g_main->selected_key_id;
}
...
void motion(GLFWwindow* window, double x, double y)
{
  ...
  zNURBS3D* nurbs = &g_main->p2p_array.buf[g_main->selected_path_id].ref_path_array.buf[g_main->selected_ik_id].nurbs;
  if( g_main->selected_cp > 0 && g_main->selected_cp < zNURBS3D1CPNum(nurbs) -1 ){
    rkglUnproject( g_cam, x, y, rkglSelectionZnearDepth(&g_main->sb), &p );
    zVec3DCopy( &p, zNURBS3D1CP(nurbs, g_main->selected_cp) );
...
{
  const int path_size = zArraySize( &g_main->p2p_array );
  const double end_s = g_main->p2p_array.buf[path_size-1].goal_feedrate.s;
  ...
  case 'f': change_pose( g_main->feedrate_s + ds); break; /* forward */
  case 'b': change_pose( g_main->feedrate_s - ds); break; /* backward */
...
void set_modelfile(char* modelfile)
{
  g_main->modelfile = modelfile;
}
...
bool init(void)
{
  if( g_main->modelfile == NULL ){
    g_main->modelfile = zStrClone( "../model/puma.ztk" );
  }
  printf( "modelfile = %s\n", g_main->modelfile );
  if( !extend_rkChainReadZTK( &g_main->chain, g_main->modelfile ) ){
    ZRUNWARN( "Failed extend_rkChainReadZTK()" );
    return false;
  }
  rkglChainAttr attr;
  rkglChainAttrInit( &attr );
  if( !rkglChainLoad( &g_main->glChain, &g_main->chain, &attr, g_light ) ){
    ZRUNWARN( "Failed rkglChainLoad(&g_main->glChain)" );
    return false;
  }
  /* IK */
...
{
  /* First of All, create pindragIFData g_main */
  void* test_main_ptr = NULL;
  create_empty_motionPathViewData( &test_main_ptr );
  set_motionPathViewData( test_main_ptr );

  int width;
  int height;

  if( argc > 1 ){
    set_modelfile( argv[1] );
  }
  ...
  destroy_motionPathViewData( test_main_ptr );
